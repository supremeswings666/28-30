...
exports.getAll = ({ page = 1, limit = 10, email = null }) => {
  ...
  let query = `SELECT id, name, email, phone, registeredAt FROM customers`;
  const params = [];
...
exports.getById = (id) => {
  const stmt = db.prepare(
    `SELECT id, name, email, phone, registeredAt FROM customers
        WHERE id = ?`
  );
  return stmt.get(id);
};
...
exports.create = (data) => {
  ...
  const existing = db
    ...
    .get(email);
  if (existing) {
    throw new AppError("Klient É takim email yxe cyuectbyet", 400);
  }
...
exports.update = (id, data) => {
  ...
  if (email) {
    const existing = db
      ...
      .get(email, id);
    if (existing) {
      throw new AppError("Клиент 8 таким email уже существует", 400);
    }
    updates.push("email = ?");
    params.push(email);
  }
